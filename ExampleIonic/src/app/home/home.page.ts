...
export class HomePage {
  ...
  prependLogMessage(message: any){
    this.#output.innerHTML = this.#output.innerHTML +
        '<span class="'+(message.logLevel ? message.logLevel : '')+'">'
            +message.msg + '</span>' + (message.nobreak ? "<br/>" : "<br/><br/>" );
  }
  ...
  constructor(private platform: Platform,
      ...
      platform.ready().then(() => {
        this.retenoPlugin.setApiKey("630A66AF-C1D3-4F2A-ACC1-0D51C38D2B05").then(
          (response)=> {console.log("set api key" + response);
        });
        ...
        this.firebasePlugin.registerAuthStateChangeListener(this.registerAuthStateChangeListener.bind(this));
  ...
  registerAuthStateChangeListener(userSignedIn: boolean) {
    this.log("Auth state changed: User signed " + (userSignedIn ? "in" : "out"), undefined);
  }
  ...
  getID() {
    this.firebasePlugin.getId().then((id) => {
  ...
  getAPNSToken() {
    this.firebasePlugin.getAPNSToken().then((token)=>{
  ...
  getBadgeNumber() {
    this.firebasePlugin.getBadgeNumber().then((current: number)=> {
  ...
  private checkNotificationPermission(requested: boolean) {
    this.firebasePlugin.hasPermission().then ( (hasPermission:boolean)=>{
      if(hasPermission){
        ...
        this.firebasePlugin.grantPermission().then(this.checkNotificationPermission.bind(this, true));
      }else{
        // Denied
        this.logError("Notifications won't be shown as permission is denied", undefined, false);
      }
    });
  }

  private checkAutoInit(showUser: boolean) {
    this.firebasePlugin.isAutoInitEnabled().then ((enabled: boolean)=>{
      this.log("Auto init is " + (enabled ? "enabled" : "disabled"), showUser);
    }, (error: any) => {

...
function alertUser(title, msg, cb) {
  navigator.notification.alert(msg, cb, title);
}
...
function onDeviceReady() {
  ...
  RetenoPlugin.setApiKey(
    "630A66AF-C1D3-4F2A-ACC1-0D51C38D2B05",
    function (msg) {},
    function (err) {}
  );
...
var initIos = function () {
  ...
  FirebasePlugin.registerInstallationIdChangeListener(function (
    installationId
  ) {
...
function setCrashlyticsUserId() {
  FirebasePlugin.setCrashlyticsUserId(
    "crashlytics_user_id",
    function () {
...
function setAnalyticsCollectionEnabled() {
  FirebasePlugin.setAnalyticsCollectionEnabled(
    true,
    function () {
...
function setPerformanceCollectionEnabled() {
  FirebasePlugin.setPerformanceCollectionEnabled(
    true,
    function () {
...
function incrementCounter() {
  FirebasePlugin.incrementCounter(
    traceName,
    "my_counter",
    function () {
  ...
  FirebasePlugin.getValue(
    ...
    function (value) {
      try {
        ...
        return logError(
          "Failed to parse JSON value from remote config",
          e.message,
          true
        );
      }
...
function verifyPhoneNumber() {
  ...
  var enterPhoneNumber = function () {
    promptUserForInput(
      "Enter phone number",
      "Input full phone number including international dialing code",
  ...
  var enterVerificationCode = function (credential) {
    promptUserForInput(
      ...
      "Input the code in the received verification SMS",
      function (verificationCode) {
        if (!verificationCode)
          return alertUser(
            "verification code",
            "Valid verification code must be entered",
  ...
  var verify = function () {
    var fakeVerificationCode = $(
        "#enterPhoneNumber .mockInstantVerificationInput"
      )[0].checked
        ? FAKE_SMS_VERIFICATION_CODE
        : null,
      requireSmsValidation = $(
        "#enterPhoneNumber .requireSmsValidationInput"
      )[0].checked;
      ...
      function (credential) {
        ...
        if (credential.instantVerification) {
          if (awaitingSms) {
            awaitingSms = false;
            log(
              "Auto-retrieval used to retrieve verification code from SMS - user does not need to manually enter"
            );
...
function enrollSecondAuthFactor() {
  ...
  var enterDisplayName = function () {
    promptUserForInput(
      "Enter display name",
      'Input name for second factor e.g. "Work phone" (or leave blank)',
  ...
  var enroll = function () {
    ...
    var fakeVerificationCode = $(
        "#enrollSecondAuthFactor .mockInstantVerificationInput"
      )[0].checked
        ? FAKE_SMS_VERIFICATION_CODE
        : null,
      requireSmsValidation = $(
        "#enrollSecondAuthFactor .requireSmsValidationInput"
      )[0].checked;
...
function verifySecondAuthFactor() {
  if (!_secondFactors)
    return logError("No second factors found to selected from!", true);
  var selectedIndex,
    credential,
  ...
  var selectFactor = function () {
    ...
    promptUserForInput(
      "Enter factor number",
      msg,
      function (enteredFactorNumber) {
        if (
          !isNumericString(enteredFactorNumber) ||
          !_secondFactors[enteredFactorNumber - 1]
        )
          return alertUser(
            "Invalid factor",
            "A factor number between 1 and " +
              _secondFactors.length +
              " must be entered",
            selectFactor
          );
  ...
  var enterPhoneNumber = function () {
    promptUserForInput(
      ...
      "Input test phone number for fake instant verification",
      function (_phoneNumber) {
        if (!_phoneNumber)
          return alertUser(
            "Invalid phone number",
  ...
  var confirmUseFakeVerificationCode = function () {
    promptUserForYesNoChoice(
      "Use fake verification code?",
      "Test instant verification on Android using a test phone number?",
      function (shouldUse) {
  ...
  var confirmRequireSMSValidation = function () {
    promptUserForYesNoChoice(
      ...
      "Always require SMS validation on Android even if instant verification is available?",
  ...
  var verify = function () {
    ...
    if (cordova.platformId === "android") {
      if (typeof fakeVerificationCode === "undefined")
        return confirmUseFakeVerificationCode();
      if (typeof requireSmsValidation === "undefined")
        return confirmRequireSMSValidation();
    }
...
function unenrollSecondFactor() {
  ...
  function selectFactor() {
    ...
    promptUserForInput(
      "Enter factor number to unenroll",
      msg,
      function (enteredFactorNumber) {
        if (
          !isNumericString(enteredFactorNumber) ||
          !secondFactors[enteredFactorNumber - 1]
        )
          return alertUser(
            "Invalid factor",
            "A factor number between 1 and " +
  ...
  function unenroll(selectedIndex) {
    FirebasePlugin.unenrollSecondAuthFactor(
      ...
      function (error) {
        logError(
          "Failed to unenroll second factor: " + JSON.stringify(error),
          true
        );
      },
      selectedIndex
...
function signInWithCredential() {
  if (!authCredential)
    return logError("No auth credential exists - request a credential first");
...
function linkUserWithCredential() {
  if (!authCredential)
    return logError(
      "No auth credential exists - request a credential first",
      true
    );
...
function updateUserProfile() {
  ...
  var inputPhotoUri = function () {
    promptUserForInput(
      "Enter photo URL",
      "Input URL for user profile photo",
      function (url) {
...
function sendUserPasswordResetEmail() {
  promptUserForInput(
    "Enter email",
    "Input user email address for reset password",
    function (email) {
      ...
      promptUserForInput(
        ...
        function (password) {
          FirebasePlugin.createUserWithEmailAndPassword(
            ...
            function () {
              log(
                "Successfully created email/password-based user account",
                true
              );
            },
            function (error, secondFactors) {
    ...
    promptUserForInput(
      ...
      function (password) {
        FirebasePlugin.signInUserWithEmailAndPassword(
          ...
          function () {
            log(
              "Successfully signed in to email/password-based user account",
              true
            );
          },
          function (error, secondFactors) {
            ...
            logError(
              "Failed to sign in to email/password-based user account",
              error,
              true
            );
          }
...
function setDocumentInFirestoreCollection() {
  FirebasePlugin.setDocumentInFirestoreCollection(
    ...
    function () {
      log(
        "Successfully set document in Firestore with id=" + firestoreDocumentId,
        true
      );
    },
    function (error) {
...
function updateDocumentInFirestoreCollection() {
  ...
  FirebasePlugin.updateDocumentInFirestoreCollection(
    ...
    function () {
      log(
        "Successfully updated document in Firestore with id=" +
          firestoreDocumentId,
        true
      );
    },
...
function deleteDocumentFromFirestoreCollection() {
  FirebasePlugin.deleteDocumentFromFirestoreCollection(
    ...
    function () {
      log(
        "Successfully deleted document in Firestore with id=" +
          firestoreDocumentId,
        true
      );
    },
...
function fetchDocumentInFirestoreCollection() {
  FirebasePlugin.fetchDocumentInFirestoreCollection(
    ...
    function (document) {
      log(
        "Successfully fetched document in Firestore with id=" +
          firestoreDocumentId +
          "; doc=" +
...
function fetchFirestoreCollection() {
  var filters = [["where", "an_integer", "==", 1, "integer"]];
  FirebasePlugin.fetchFirestoreCollection(
    firestoreCollection,
    filters,
    function (data) {
      log(
        "Successfully fetched Firestore collection: " + JSON.stringify(data),
        true
      );
      console.dir(data);
    },
  ...
  FirebasePlugin.listenToDocumentInFirestoreCollection(
    function (documentEvent) {
      if (documentEvent.eventType === "id") {
        ...
        log(
          "Listening for document changes in Firestore with id=" +
            documentListenerId,
          true
        );
      } else {
        log(
          "Document change detected: document=" +
            firestoreDocumentId +
            "; collection=" +
            firestoreCollection +
            " changes=" +
            JSON.stringify(documentEvent)
        );
    ...
    function (error) {
      logError(
        "Failed to listen for changes to document in Firestore",
        error,
        true
      );
    },
...
function unlistenToDocument() {
  ...
  FirebasePlugin.removeFirestoreListener(
    ...
    function (error) {
      logError(
        "Failed to stop listening for changes to document in Firestore",
        error,
        true
      );
    },
  ...
  FirebasePlugin.listenToFirestoreCollection(
    function (collectionEvent) {
      if (collectionEvent.eventType === "id") {
        ...
        log(
          "Listening for collection changes in Firestore with id=" +
            collectionListenerId,
          true
        );
      } else {
        log(
          "Collection change detected: collection=" +
            firestoreCollection +
            " changes=" +
    ...
    function (error) {
      logError(
        "Failed to listen for changes to collection in Firestore",
        error,
        true
      );
    },
...
function unlistenToCollection() {
  ...
  FirebasePlugin.removeFirestoreListener(
    ...
    function (error) {
      logError(
        "Failed to stop listening for changes to collection in Firestore",
        error,
        true
      );
    },
...
function callHttpsFunction() {
  ...
  FirebasePlugin.functionsHttpsCallable(
    ...
    function (result) {
      log(
        "Successfully called function - result: " + JSON.stringify(result),
        true
      );
    },
    function (error) {
